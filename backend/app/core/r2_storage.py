...
# Layout::
#
#   pending-tl-screenshots/<request_id>-<a|b>.png         -- raw upload
#   pending-tl-screenshots/<request_id>-<a|b>-minimap.png -- detected minimap crop
#
# All deleted on approve / reject — never archived.

TL_SCREENSHOT_PREFIX = "pending-tl-screenshots/"


def tl_screenshot_pending_key(request_id: str, slot: str) -> str:
    """R2 key for the raw screenshot uploaded by a contributor.

    ``slot`` must be ``"a"`` or ``"b"``.
    """
    if slot not in ("a", "b"):
        raise ValueError(f"slot must be 'a' or 'b', got {slot!r}")
    return f"{TL_SCREENSHOT_PREFIX}{request_id}-{slot}.png"


def tl_screenshot_minimap_crop_key(request_id: str, slot: str) -> str:
    """R2 key for the edge-detected minimap crop the admin sees in the
    review dialog. Re-generated by the analysis worker."""
    if slot not in ("a", "b"):
        raise ValueError(f"slot must be 'a' or 'b', got {slot!r}")
