...
from .routes import admin_program as admin_program_routes
from .routes import public_program_download
...
async def lifespan(app: FastAPI):
    ...
    # Debounced coalescer that rebuilds + republishes the public Auction House
    # artifacts after auction contributions are ingested.
    step_started = perf_counter()
    try:
        from .core import auction_rebuild
        auction_rebuild.start()
        logger.info(
            "Startup step auction_rebuild coalescer started in %.3fs",
            perf_counter() - step_started,
        )
    except Exception as exc:  # pragma: no cover
...
app.include_router(admin_program_routes.router, prefix="/api")
app.include_router(public_program_download.router, prefix="/api")
