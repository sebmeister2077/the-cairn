...
export function ContributionRegionPicker({
    ...
    const display = useMemo(() => {
        ...
        const ratio = Math.min(
            MAX_DISPLAY_DIM / level.image_w,
            MAX_DISPLAY_DIM / level.image_h,
            1,
        );
        return {
    ...
    if (availableLevels.length === 0) {
        return (
            <div className="rounded border border-dashed p-3 text-sm text-muted-foreground">
                No TOPS map preview is available yet — the region picker needs at
                least one generated map level.
            </div>
        );
    }
    if (!targetLevel) {
    ...
    const selectionTiles = value
        ? regionTileArea(value, tileSizeBlocks)
        : 0;
    const overCap =
        tileAreaCap != null && selectionTiles > tileAreaCap;
    ...
    return (
        <div className="space-y-2">
            <div className="text-sm">
                ...
                {!loading && !error && (
                    <span>
                        Drag on the map to select the region your upload should
                        overwrite. Outside the rectangle, existing tiles stay
                        untouched.
                    </span>
                )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 text-sm">
                {value ? (
                    <>
                        <span>
                            Region: x [{value.min_x}, {value.max_x}], z [
                            {value.min_z}, {value.max_z}] blocks
                        </span>
                    ...
                    <span className="text-muted-foreground">
                        No region selected — your upload will gap-fill
                        unmapped tiles only (legacy mode).
                    </span>
                )}
            </div>
        </div>
...
function regionTileArea(region: ContributionRegion, tileSize: number): number {
    ...
    return (
        Math.max(0, tx_max - tx_min + 1) * Math.max(0, tz_max - tz_min + 1)
    );
}
