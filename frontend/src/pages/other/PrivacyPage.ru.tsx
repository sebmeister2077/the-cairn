...
const LAST_UPDATED = "7 августа 2026";
    ...
    <Card>
      ...
      <CardContent className="space-y-6 text-sm leading-relaxed text-muted-foreground">
        ...
        <section className="space-y-2">
          ...
          <p>
            До того как вы примете уведомление о хранении, мы держим в браузере{" "}
            <strong>только ваш API-ключ</strong> (он нужен для аутентификации и ограничения частоты
            запросов). Остальные предпочтения и кэш сохраняются только{" "}
            <strong>после согласия</strong>; некоторые функции, которые опираются на сохранённые
            данные (например, избранные группы транслокаторов), до этого недоступны.
          </p>
          <p>
        ...
        <section className="space-y-2">
          <h2 className="text-base font-semibold text-foreground">
            4e. Сообщество: маркетплейс (Orders)
          </h2>
          <p>
            Если маркетплейс <strong>Orders</strong> включён, владельцы аккаунтов могут размещать
            заявки на покупку/продажу и договариваться о сделках. Эта функция хранит контент,
            который создаёте <strong>вы</strong>:
          </p>
          <ul className="list-disc pl-5 space-y-1">
            <li>
              <strong>Ваши заявки</strong> — предмет, сторона (покупка/продажа), цена за единицу,
              количество и любой произвольный текст в полях краткого описания и заметок. Открытые
              заявки <strong>публичны</strong> и показываются рядом с вашим публичным display name
              (или in-game name, если вы его включили).
            </li>
            <li>
              Необязательное <strong>местоположение торговца</strong> (внутриигровые координаты X/Z,
              необязательная метка или ссылка на один из ваших landmark) и настройка доступности,
              если вы решите их добавить.
            </li>
            <li>
              <strong>Запросы и сообщения переговоров</strong> — когда вы открываете запрос или
              отвечаете на него, мы храним предложенное количество/цену и произвольную заметку.
              Ветка переговоров видна только двум её участникам (инициатору запроса и владельцу
              заявки), а не всем.
            </li>
            <li>
              <strong>Отметки о состоявшихся сделках (fills)</strong>, которые фиксирует владелец
              заявки (количество, необязательная цена и признак публикации), чтобы построить график
              цен по заявке.
            </li>
          </ul>
          <p>
            Пожалуйста, не указывайте персональные данные в этих полях свободного текста. Вы можете{" "}
            <strong>в любой момент удалить свои заявки</strong> со страницы заявки — это навсегда
            удаляет заявку вместе со всей историей переговоров. Администраторы также могут удалить
            любую заявку или сообщение, нарушающее Условия. Поскольку сделки заключаются напрямую
            между пользователями, оператор не является их стороной — см.{" "}
        ...
        <section className="space-y-2">
          <h2 className="text-base font-semibold text-foreground">
            4f. Auction House и рыночные данные
          </h2>
          <p>
            Страницы <strong>Auction House</strong> и связанные рыночные разделы (скринер, страницы
            предметов и игроков, таблицы лидеров, оценка редкости) показывают статистику, собранную
            из внутриигровых записей аукциона, переданных проекту. Эти игровые данные могут включать{" "}
            <strong>внутриигровые имена персонажей</strong> и цены/количества их сделок, и они могут
            отображаться на публичных страницах, таких как профили игроков и таблицы лидеров. Это
            отражает публичную внутриигровую активность, а не сведения, которые вы нам
            предоставляете. Если вы считаете, что имя персонажа, идентифицирующее вас, следует
            удалить, напишите нам по адресу из раздела 13.
          </p>
        </section>

        <section className="space-y-2">
          <h2 className="text-base font-semibold text-foreground">
            4g. Инструменты и калькуляторы
          </h2>
          <p>
            <strong>Инструменты</strong> (например, планировщик туннелей и конструктор макросов
            путевых точек) и калькуляторы <strong>редкости</strong> работают в вашем браузере. Они
            не требуют аккаунта и не собирают персональные данные сверх стандартных метаданных
            запроса и внутреннего usage log, описанных в разделах 4 и 4c.
          </p>
        </section>
        ...
        <section className="space-y-2">
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li>
              Контент маркетплейса <strong>Orders</strong> (заявки, запросы, сообщения переговоров,
              fills): хранится, пока вы не удалите заявку или её не удалит администратор. Удаление
              заявки также удаляет историю переговоров. Удаление аккаунта не удаляет автоматически
              оставленные вами заявки — удалите их сами или попросите нас.
            </li>
            <li>
              Данные Auction House / рынка: хранятся как часть общего набора рыночных данных, чтобы
              тренды оставались сопоставимыми во времени.
            </li>
            <li>
        ...
        <section className="space-y-2">
          ...
          <ul className="list-disc pl-5 space-y-1">
            ...
            <li>
              ...
              выполняет soft-delete, описанный в разделе 8. Вы также можете{" "}
              <strong>удалить любую свою заявку</strong> (и её историю переговоров) прямо со
              страницы заявки.
            </li>
          </ul>
          <p>
