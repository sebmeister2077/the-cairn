...
- [x] wrap every route in a try-except block to catch and log exceptions
-[x] add logging configuration to log errors to a file (database maybe? and local log file as fallback)
...
-[x]0 Automatically give a score based on already existing map data (Spawn). Display the amount of chunks are overlapping & the overall similarity of the overlapping chunks (ex:300 chunks with 85% resemblance)
-[x] Automatically display the regions with the most overlapping chunks (ex: top 5 regions with the most overlapping chunks) and also display for each region the similarity score of the overlapping chunks
-[x] custom domain
